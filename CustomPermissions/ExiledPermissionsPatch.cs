...
namespace CustomPermissions
{
    ...
    public static class ExiledPermissionsPatch
    {
        public static bool Prefix(Player player, string permission, ref bool __result)
        {
            ...
            Log.Debug($"UserID: {player.UserId} | PlayerId: {player.Id}", Exiled.Loader.Loader.ShouldDebugBeShown || Plugin.Singleton.Config.IsDebugged);
            Log.Debug($"Permission string: {permission}", Exiled.Loader.Loader.ShouldDebugBeShown || Plugin.Singleton.Config.IsDebugged);
            ...
            Log.Debug($"GroupKey: {plyGroupKey}", Exiled.Loader.Loader.ShouldDebugBeShown || Plugin.Singleton.Config.IsDebugged);

            if(plyGroupKey == null || !Permissions.Groups.TryGetValue(plyGroupKey, out var group))
            {
                Log.Debug("The source group is null, the default group is used", Exiled.Loader.Loader.ShouldDebugBeShown || Plugin.Singleton.Config.IsDebugged);
                group = Permissions.DefaultGroup;
            }
            if(group is null)
            {
                Log.Debug("There's no default group, returning false...", Exiled.Loader.Loader.ShouldDebugBeShown || Plugin.Singleton.Config.IsDebugged);
                __result = false;
                return false;
            }
